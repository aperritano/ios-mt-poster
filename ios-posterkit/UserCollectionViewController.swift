...
class UserCollectionViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, WYPopoverControllerDelegate {
    ...
    override func viewDidAppear(animated: Bool) {
        ...
        SwiftEventBus.onMainThread(self, name: "UserReloadedEvent") {
            _ in
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        if segue.identifier == "posterSegue" {
            ...
            let user = DBHelper.sharedMonitor().allUsers.filter() {
                $0.name == cell.title.text
            }.first!
    ...
    @IBAction func DeleteCellAction(sender: MKButton) {
        ...
        SweetAlert().showAlert("Are you sure?", subTitle: "This user and all of its posters will be permanently deleted!", style: AlertStyle.Warning, buttonTitle: "Cancel", buttonColor: UIColor.paperColorBlue400(), otherButtonTitle: "YES", otherButtonColor: UIColor.paperColorRed()) {
